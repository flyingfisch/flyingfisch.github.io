...
Now let's get the list-items to line up horizontally.
...
Those links are hard to read on the blue, let's style them up a little.
...
Let's make some HTML for our page. We know we are going to have a header and some text, so let's add that ([greeking](https://en.wikipedia.org/wiki/Greeking) was generated by [Tolkien Ipsum](http://flyingfisch.github.io/announcement/2015/05/28/tolkien-ipsum.html)).
...
Now it's time to style this stuff! The first thing I am going to do is make `h1` and `h2` small-caps.
...
As you may have noticed int the 
